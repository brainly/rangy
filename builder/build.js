...
var buildSpec = {
  gitUrl: "https://github.com/brainly/rangy.git",
  gitBranch: "master",
};

var buildDir = "dist/";

var gitDir = buildDir + "repository/",
  srcDir = gitDir + "src/";
var zipDir;
...
function assembleCoreScript() {
  ...
  fileNames.forEach(function (fileName) {
    files[fileName] = fs.readFileSync(
      srcDir + "core/" + fileName,
      FILE_ENCODING
    );
  });

  // Substitute scripts for build directives
  var combinedScript = files["core.js"].replace(
    /\/\*\s?build:includeCoreModule\((.*?)\)\s?\*\//g,
    function (match, scriptName) {
      return indent(files[scriptName]);
    }
  );

  fs.writeFileSync(
    uncompressedBuildDir + coreFilename,
    combinedScript,
    FILE_ENCODING
  );

  // Copy the types definition file
  fs.copyFileSync(
    srcDir + "core/rangy.d.ts",
    uncompressedBuildDir + "rangy.d.ts"
  );
...
function copyModuleScripts() {
  modules.forEach(function (moduleFile) {
    var moduleCode = fs.readFileSync(
      srcDir + "modules/" + moduleFile,
      FILE_ENCODING
    );

    // Run build directives
    moduleCode = moduleCode.replace(
      /\/\*\s?build:modularizeWithRangyDependency\s?\*\/([\s\S]*?)\/\*\s?build:modularizeEnd\s?\*\//gm,
      function (match, code) {
        ...
        return (
          [
            ...
            "        // Node/CommonJS style",
            '        module.exports = factory( require("@brainly/rangy") );',
            "    } else {",
    ...
    fs.writeFileSync(
      uncompressedBuildDir + moduleFile,
      moduleCode,
      FILE_ENCODING
    );
  });
...
function substituteBuildVars() {
  ...
  function doSubstituteBuildVars(file, buildVars) {
    ...
    contents = contents.replace(
      /\/\*\s?build:replaceWith\((.*?)\)\s?\*\/.*?\*\s?build:replaceEnd\s?\*\//g,
      "$1"
    );
...
function minify() {
  ...
  function uglify(src, dest) {
    ...
    return terser
      .minify(fs.readFileSync(src, FILE_ENCODING), terserOptions)
      .then(function (final_code) {
        fs.writeFileSync(
          dest,
          licence + "\r\n" + final_code.code,
          FILE_ENCODING
  ...
  Promise.all(
    allScripts.map(function (fileName) {
...
var sourceFilesGetter =
  process.argv.length >= 3 && process.argv[2] == "freshCheckout"
    ? cloneGitRepository
    : copyLocalSourceFiles;
